define("manager/EditMask", ["dojo/_base/declare"], function(declare) {
    return declare("Manager.EditMask",[],{
        version: '0.1',
        h1: null, // keypress
        h2: null, // focus
        h3: null, // submit
        ...
        constructor: function (element, mask, optional, errMsg) {
            this.element = manager.getElementById(element);
        ...
        filterExit: function(filterTemp) {
            mask = this.mask + '_';
            var filled = true;
            for (var i = 0; i < filterTemp.length ; i++ )
            {
                filled = filled && (filterTemp.charAt(i) != '_' );
            }
            if (!filled)
            {
